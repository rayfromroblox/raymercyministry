  ...
  return (
    <section id="contact" className="section-padding bg-[#F5F0E8] overflow-hidden" ref={ref}>
      <div className="container-custom">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-24 items-center">
          {/* Invitation Copy */}
          <motion.div
          ...
          >
            <div className="space-y-4 md:space-y-6">
              <div className="inline-flex items-center justify-center lg:justify-start gap-3 md:gap-4 group w-full lg:w-auto">
                ...
                <div className="h-px w-8 md:w-10 bg-[#C85A4A]" />
              </div>
              <h2 className="text-3xl xs:text-4xl md:text-8xl font-serif text-[#1A1A1A] leading-[1.1] md:leading-[0.95] tracking-tighter mx-auto lg:mx-0 max-w-lg lg:max-w-none">
                Let’s change <br />
                the <span className="text-[#C85A4A] italic font-normal">narrative.</span>
              </h2>
            </div>

            <p className="text-sm md:text-xl text-[#4A4A4A] leading-relaxed font-light max-w-xl mx-auto lg:mx-0 px-4 md:px-0">
            ...
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-4 px-4 md:px-0">
              <a href="tel:+254721826905" className="btn-primary w-full sm:w-auto text-xs md:text-sm py-3 md:py-4">
                Start a Conversation
              </a>
              <a href="mailto:rachelmacharia80@gmail.com" className="btn-secondary w-full sm:w-auto text-xs md:text-sm py-3 md:py-4">
                Send an Email
              </a>
            </div>
          </motion.div>
          ...
          >
            <div className="relative">
              ...
              <div className="bg-white rounded-[1.5rem] md:rounded-[4rem] p-6 md:p-14 shadow-[0_60px_120px_-30px_rgba(0,0,0,0.06)] border border-white/60 relative z-10 mx-2 md:mx-0">
                  ...
                  {info.map((item, i) => (
                    <div key={i} className="flex gap-4 md:gap-8 items-center group">
                      ...
                      <div className="space-y-0.5 md:space-y-1 min-w-0 flex-1">
                        ...
                        {item.link ? (
                          <a href={item.link} className="text-sm md:text-2xl font-serif font-black text-[#1A1A1A] hover:text-[#C85A4A] transition-colors leading-tight block break-all md:break-normal">
                            {item.value}
                          </a>
                        ) : (
                          <p className="text-sm md:text-2xl font-serif font-black text-[#1A1A1A] leading-tight break-words">{item.value}</p>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-8 md:mt-12 pt-6 md:pt-10 border-t border-[#F0F0F0] flex items-center gap-4 md:gap-6">
                  <div className="w-8 h-8 md:w-12 md:h-12 rounded-full bg-[#1A1A1A] flex items-center justify-center text-white text-xs md:text-base flex-shrink-0">
                    ♡
                  </div>
